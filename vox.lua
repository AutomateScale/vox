...
local C = {
  ...
  screenRecFraming    = "square",      -- wide | square | tall
  screenRecFollow     = false,         -- optional deadzone follow-me crop
...
local PREFS = { "holdKeycode", "holdKeyName", "duckMode", "duckAudio", "convLive", "alienPlayByPlay",
                ...
                "screenRecWebcamSize", "screenRecShape", "screenRecFraming", "screenRecFollow" }
...
local function transcribe()
  ...
  local function buildCmdPort(port, timeLimit)
    return string.format(
      ...
      "'BEGIN{ if(a==\"\")a=1; if(d==\"\")d=9; exit (a+0<0.006 || d+0<0.45) ? 0 : 1 }'" ..
      " && exit 42; " ..
...
<div class="card"><h2>Voom — Screen &amp; Presenter</h2>
  ...
  <div class="row"><span>Presenter style</span><select onchange="send('screenRecBgMode', this.value)">
    <option value="cutout"%s>✨ AI cutout — clean</option><option value="mint"%s>🌈 Cutout + glow</option><option value="portal"%s>🌀 Portal — dimension door</option><option value="portalcam"%s>⭕ Portal — no cutout</option>
  ...
  <div class="row"><span>Framing</span><select onchange="send('screenRecFraming', this.value)">
    <option value="wide"%s>Wide 16:9</option><option value="square"%s>Square</option>
    <option value="tall"%s>Tall</option></select></div>
...
window.addEventListener('keydown', function(e) {
  ...
  recording.btn.classList.remove('rec'); recording = null;
    ...
    sel(C.screenRecBgMode, "portal"), sel(C.screenRecBgMode, "portalcam"),
